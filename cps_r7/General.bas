...
'            _a Added check for WB temp still OK at end of Manual Gas Pause   ....................11     April 2018
...
'            _a Added PAS AK Interface  ..........................................................25     April 2019
'            _a Added PAS AK Server Logging  (cpsZlog_rev0703; added AK_Log table)................28     April 2019
'            _a Replaced SSCommand on frmAbout with Image Control ................................21       May 2019
'            _a BugFix: Config screen Purge Air Temp Tooltip range ...............................25      June 2019
'            _a BugFix: Display of Test Est Duration & Time So Far ...............................25      June 2019
'            _a BugFix: Recipe screen Uncheck Load-By-PID when Using-Live-Fuel is unchecked ......25      June 2019
'            _a Added PAS AK Interface Alarm Indicator to Main ...................................27      June 2019
'            _a Added Compact Master DB File at program startup ..................................23      July 2019
'            _a Added PAS AK Interface Alarm Indicator to Detail,Joblist, Review & Watcher .......13    August 2019
'            _a Added choice of "xls files" to CopyFiles screen ..................................30    August 2019
'            _a Added Remote Status Monitoring choice to sysdef ...................................2 September 2019
'            _a Added Using-AVL-File-Remote-Interface choice to sysdef ............................2 September 2019
'            _a Added Station Abreviation and Station SystemID to STN_INFO ........................3 September 2019
'            _a Added Remote Status Monitoring Interval to config (cpsRemote_rev0711)..............5 September 2019
'            _a Added Remote Status Monitoring Interval to config (cpsModel_rev0740)...............5 September 2019
'            _a Added support for AVL-Host-Task-Order-Files .......................................5 September 2019
'            _a Added support for AVL-Host-Unit-Status-Files ......................................6 September 2019
'            _a Added support for AVL-Host-Task-Order retest logic ...............................13 September 2019
'            _a Added support for High Temperature PAS ...........................................18 September 2019
'            _a BugFix: Individual Update of Remote Can,Rcp,Prf,Seq or Cfg .......................26 September 2019
'            _a BugFix: Display of Canister Volume on RemoteTask screen ...........................3   October 2019
'            _a Changed: AVL-Host-Unit-Status-Files always use txt extention ......................7   October 2019
'            _a Added: Alarm Desc (when in alarm pause) to AVL-Host-Unit-Status-Files ............16   October 2019
'            _a Added: Request-to-Set-InActive option to AVL-Host-Task-Order-Files ...............16   October 2019
'            _a Added: Save (& Load) to Registry of Startup Verbose Msgs .........................22   October 2019
'            _a Changed: AVL-Host-Unit-Status-Files no longer use quotation marks ................28   October 2019
'            _a Changed: AVL-Host-Task-Order-Files no longer use quotation marks .................28   October 2019
'            _a BugFix: RemoteTaskFile Status when Oper stops job ................................28   October 2019
'            _a BugFix: AVL Status File update Timer doesn't rollover properly ...................29   October 2019
'RELEASE 7.07.04 .................................................................................31   October 2019
'            _a BugFix: ADF Refill then Dump when Load/Purge ......................................7  November 2019
'RELEASE 7.07.05
'            _a Update for BMW
'            _a Added in TOM
'
'            _a Repaired Leak Test
'
'
'
...
Global Const VER_DESCRIPTION = "Debug Version  for  BMW "                'Debug Version
Global Const PROGRAMREVLVL = "7.07.04"          ' Revision Level of Cps_r7 program
Global Const USINGRELEASEDATE = VER_DESCRIPTION & PROGRAMREVLVL & "  2019 November 7"   ' CHANGE DATE AS REQUIRED
...
Global Const DBMODELREVLVL = 40                 ' revision sublevel for Model DB file
...
Global Const DBTOMREVLVL = 2                    ' revision sublevel for TOM Tasks DB file
Global Const DBREMREVLVL = 11                   ' revision sublevel for Remote Tasks DB file
...
Global Const icPASPowerOnIn = 30                    ' Index into the Common Digital IO Functions
Global Const icPASReadyOut = 31                     ' Index into the Common Digital IO Functions
Global Const icPASisRunningIn = 33                   ' Index into the Common Digital IO Functions
...
'                  20 = ADF #4; Pump, Drain, Fill, Vapor, Heater, LT                    (first=Stant;Fall2016)
...
Type StnInfo
    ...
    SysID As String                                                  ' Station Text System Unique ID (alphanumeric)
    Abrev As String                                                  ' Station single character abreviation
...
' *************************************************************************************
' Declare Task Order Manager (TOM) DataType
'
'
Type TomData
    TaskID As String
    ...
    CanWC As Single
    CanVolume As Single
    rcpUse2Gm As Boolean
    rcpUseWCM As Boolean
    rcpRecipeNumber As Integer
    RequestedStation As Integer
    RequestedShift As Integer
    ActualStation As Integer
    ActualShift As Integer
End Type
Global CurTomTask As TomData
Global GenTomTask As TomData
Global StnTomTask(1 To MAX_STN, 1 To MAX_SHIFT) As TomData


' *************************************************************************************
' Declare Remote Task DataType
'
'
Type RemTaskControlBlock
    TaskID As String
    TaskCaption As String
    VIN As String
    TaskStatus As String
    PreviousResult As String
    ReportComplete As Boolean
    ...
    OrderDate As Date
    JobNumber As String
    InhibitChanges As Boolean
    AVL_FileRoot As String
End Type
Global CurRemoteTask As RemTaskControlBlock
Global StnRemoteTask(0 To MAX_STN, 0 To MAX_SHIFT) As RemTaskControlBlock
...
Global iSeq As JobSequence

'  #####  ADDED
' *************************************************************************************
' Report Generation Header data
Type HeaderData
    JobNumber As String * 6
    JobDescription As String * 50
    station As Integer
    Shift As Integer
    Engineer As String * 40
    Vehicle As String * 40
    StartOp As String * 40
    EndOp As String * 40
    ...
    EndBaro As Single
    BeginTime As Date
    DelayEndTime As Date
    EndTime As Date
    EndOK As Boolean
    StnType As Integer
    AdfTankType As Integer
    PurgeMfcEUMax As Single
    ButMfcEUMax As Single
    NitMfcEUMax As Single
    LiveFuelMfcEUMax As Single
    ButGramsPerLiter As Single
    ButMfcDensityMult As Single
    TomTaskID As String
    UsingAuxOutputs As Boolean
    NrAuxOutputs As Integer
    AuxOutDesc1 As String
    AuxOutDesc2 As String
    AuxOutDesc3 As String
    AuxOutDesc4 As String
    USINGCOMMONTC As Boolean
    USINGLINEVOLUME As Boolean
    USINGTOMCANLOAD As Boolean
End Type
Global Gen_Header As HeaderData
'  #####   ADDED
...
'
'
Type RemStatusControlBlockType
    Mode_LastStatus As Integer
    Phase_LastStatus As Integer
    Cycle_LastStatus As Integer
End Type
Global StationRemStatusControl(1 To MAX_STN, 1 To MAX_SHIFT) As RemStatusControlBlockType
' *************************************************************************************
' Declare Station Control Block DataType
'
'
...
Global Stn_RemStatus_Log_TestTimer(1 To MAX_STN, 1 To MAX_SHIFT) As Double
...
' Startup Variables
Global STARTUPVERBOSE As Boolean             ' Verbose messages during startup
Global IntroDone As Boolean                  ' Program Intro Done
Global ElogInitDone As Boolean               ' Elog Init Done
...
Global USINGHIGHTEMPPAS As Boolean           ' Using High Temperature Purge Air System(140F, 60C)
...
Global USINGREMCANLOAD As Boolean            ' REMOTE - Using Remote Task Ordering
Global USINGREMAVLFILES As Boolean           ' REMOTE - Using AVL File Interface
Global USINGREMSTSMON As Boolean             ' REMOTE - Using Remote Status Monitoring
Global REMCHGSENABLED As Boolean             ' REMOTE - Operator Changes enabled
Global TOM_2Gm_Recipe As Integer             ' TOM - Default 2 gram breakthrough recipe
Global TOM_Wcm_Recipe As Integer             ' TOM - Default Working Capacity Multiplier recipe

Global USINGTOMCANLOAD As Boolean
Global Spare0 As Boolean
Global Spare1 As Boolean
Global Spare2 As Boolean
Global Spare3 As Boolean
Global Spare4 As Boolean
Global Spare5 As Boolean
Global Spare6 As Boolean
Global Spare7 As Boolean
...
Global FILEPATH_avlhost  As String                      ' root path for all AVL Remote Host files
Global FILEPATH_avlstatus  As String                    ' path for all AVL Remote Host Status files
Global FILEPATH_avltasks  As String                     ' path for all AVL Remote Host Task files
...
Global DATATOM As String                                ' DataBase File with TOM Tasks
...
'
' General Variables
Global IoComm_Flag As Boolean                       ' I/O Communication Status, true = good
Global PaComm_Flag As Boolean                       ' PurgeAir System AK Communication Status, true = good
...
' AK Client reset sequence
Global AkClientResetStepNum As Integer
Global AkClientResetStepNumNext As Integer
Global AkClientResetStepDoneSecs As Double
Global AkClientResetStepStartSecs As Double
Global AkClientResetStepSecsSoFar As Double
...
Dim X As Printer
    ...
    frmAbout.UpdateMsg "Loading Startup Settings" & vbCrLf
    Load_StartupSettings

    ' Check required files & folders
    FilesAndFolders
           
    ' Compact master DB File
    frmAbout.UpdateMsg "Compact Master DB File" & vbCrLf
    CompactRepairAccessDB FILEPATH_sysdbf, DATAMASTER
    ...
    Delay_Box "", INTRODELAY, msgNOSHOW
    frmAbout.UpdateMsg "Load System Definition Files 01" & vbCrLf
    
    ' load screen colors
    frmAbout.UpdateMsg "Load Screen Colors" & vbCrLf
    Load_ScreenColors
    frmAbout.UpdateMsg "Load Screen Colors_01" & vbCrLf
    
    ' Load System Definition Data
    Load_SysDef
    frmAbout.UpdateMsg "Save System Definition Files" & vbCrLf
...
'        Load_FidConfig
        ...
        ' Remote DB Task & Status Connections
        If ((USINGREMCANLOAD) Or (USINGREMSTSMON)) Then frmAbout.UpdateMsg "Opening Remote DB Connections" & vbCrLf
        If USINGREMCANLOAD Then OpenConnToRemTaskDb
        If USINGREMSTSMON Then OpenConnToRemStatusDb
